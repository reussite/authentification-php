<?php
    ...
    if(isset($valider)){
        ...
        elseif(empty($pass != $repass)) $erreur = 'Mot de passe non identique!';
        else{
            require('connexion.php');
            $sel=$bdd->prepare("SELECT ID FROM utlisateurs WHERE login=? LIMIT 1");
            ...
            if (count($tab)>0) {
                ...
                $erreur = 'Login existe déjà!!';
                    ...
                    $inscription=$bdd->prepare('INSERT INTO utilisateurs(nom,prenom,login,pass) VALUE(?,?,?,?)');
                    if($inscription->execute(array($nom,$prenom,$login,md5($pass))))
